...
// JS habilitado.
// Sin contacto directo (WhatsApp) desde acá a propósito: el cliente
// pide presupuesto y el admin coordina con el manita desde /admin, así
// toda solicitud real queda registrada en jobs en vez de coordinarse
...
export default async function ManitasPage({ searchParams }: ManitasPageProps) {
  ...
  let manitasQuery = supabase
    .from("profiles")
    .select(
      "id, full_name, avatar_url, specialty, bio, coverage_zone, is_verified, years_experience, availability, role"
    )
        ...
        <div className="flex items-center gap-2">
          ...
          <MobileNavMenu
            links={[
              ...
              { href: "/manitas", label: "Nuestros manitas", active: true },
            ]}
          />
        </div>
      </header>
          ...
          <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {cards.map((manita) => (
              ...
              >
                ...
                <span className="mt-auto flex items-center gap-1 text-xs font-semibold text-brand opacity-0 transition-opacity group-hover:opacity-100">
